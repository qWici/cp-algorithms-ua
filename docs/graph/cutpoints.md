...
Нам задано <Term tip="Граф, у якому ребра не мають напрямку: якщо вершини з'єднані, переходити між ними можна в обидва боки.">неорієнтований граф</Term>. Точка зчленування (англ. articulation point, або cut vertex) — це вершина, яка при видаленні разом з <Term tip="Інцидентні вершині ребра — це всі ребра, які до неї приєднані (виходять з неї або входять у неї).">інцидентними їй ребрами</Term> робить граф незв'язним (точніше, збільшує кількість <Term tip="Компонента зв'язності — максимальна група вершин, у якій від будь-якої можна дійти до будь-якої іншої; граф може складатися з кількох таких незалежних груп.">компонент зв'язності</Term> в графі). Завдання полягає в тому, щоб знайти всі точки зчленування в заданому графі.
...
Якщо поточне ребро $(v, to)$ таке, що ані вершина $to$, ані жоден з її нащадків у дереві обходу DFS не має <Term tip="Ребро, яке під час обходу в глибину веде від поточної вершини назад до її предка в дереві обходу.">зворотного ребра</Term> до жодного з предків $v$, то $v$ є точкою зчленування. Інакше $v$ не є точкою зчленування.
