...
- Потрібні теоретично оптимальні $O(1)$ на запит LCA при лише $O(N)$ передобчисленні (онлайн, дуже багато запитів)? *(якщо запити відомі заздалегідь, простіший вибір → офлайн [Тар'ян](lca_tarjan.md))*
- Ви готові до складнішої реалізації заради цієї асимптотики? *(якщо хочете простіший онлайн-метод і вас влаштовує $O(\log N)$ на запит → [двійкові підйоми](lca_binary_lifting.md))*
- Задачу справді можна звести до <Term tip="RMQ (Range Minimum Query) — задача про швидке знаходження мінімального елемента на заданому відрізку масиву.">RMQ</Term> на масиві висот ейлерового обходу, де сусідні елементи відрізняються рівно на одиницю (це ключова умова методу)?
:::
...
На наступному малюнку ви можете бачити один із можливих <Term tip="Ейлерів обхід дерева — це послідовність вершин у порядку, у якому їх відвідує пошук у глибину, із записом вершини як при вході, так і щоразу після повернення з дитини.">ейлерових обходів</Term> графа, а в списку нижче — відвідані вершини та їхні висоти.
...
У тій статті мінімум на відрізку знаходили або <Term tip="Прийом, що ділить масив на блоки розміру приблизно корінь з N; це балансує між підготовкою та швидкістю відповіді на запити.">кореневим розбиттям</Term> за $O(\sqrt{N})$, або за $O(\log N)$ за допомогою дерева відрізків.
