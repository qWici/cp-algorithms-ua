...
- Граф <Term tip="Граф, у якому ребер відносно мало порівняно з кількістю вершин (кожна вершина з'єднана лише з кількома іншими).">розріджений</Term>, тобто $m \ll n^2$? *(якщо граф щільний, $m \approx n^2$ → простіша [версія за $O(n^2)$](dijkstra.md))*
- Усі ваги ребер невід'ємні? *(якщо є від'ємні ребра → [Беллман-Форд](bellman_ford.md))*
- Потрібні найкоротші шляхи з однієї вершини? *(якщо ваги лише $0$ та $1$ → ще швидший [0-1 BFS](01_bfs.md))*
:::
...
Найефективнішою є **<Term tip="Складна структура даних — черга з пріоритетом, що дуже швидко знаходить найменший елемент і дешево зменшує ключі; названа на честь чисел Фібоначчі, які виникають у її аналізі.">фібоначчієва купа</Term>**, яка дозволяє виконувати першу операцію за $O(\log n)$, а другу — за $O(1)$.
...
Перша базується на <Term tip="Різновид збалансованого двійкового дерева пошуку: воно саме підтримує невелику висоту, тож пошук, вставка й видалення працюють за логарифмічний час.">червоно-чорних деревах</Term>, а друга — на купах.
