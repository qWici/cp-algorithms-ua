...
**Важко-легка декомпозиція (HLD)** — це доволі загальна техніка, яка дозволяє ефективно розв'язувати багато задач, що зводяться до **<Term tip="Запити, що просять обчислити щось (суму, мінімум тощо) уздовж шляху або в частині дерева, та оновлення значень у його вершинах чи на ребрах.">запитів на дереві</Term>**.
...
- Запити змінюють значення вершин/ребер (онлайн з оновленнями), тож статичних технік недостатньо? *(якщо потрібен лише найнижчий спільний предок без агрегатів → простіший [LCA через двійкові підйоми](lca_binary_lifting.md))*
- Дерево статичне за структурою, щоб один раз розбити його на важкі шляхи й покласти зверху [дерево відрізків](../data_structures/segment_tree.md)?
:::

## Опис \{#description}

Нехай задано дерево $G$ із $n$ вершин з <Term tip="Корінь — обрана головна вершина дерева, від якої воно «звисає»; відносно неї визначають предків, нащадків і піддерева. «Довільним» означає, що коренем можна призначити будь-яку вершину.">довільним коренем</Term>.
