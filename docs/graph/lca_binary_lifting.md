...
Алгоритм, описаний у цій статті, потребуватиме $O(N \log N)$ на <Term tip="Підготовчі обчислення, які роблять один раз до запитів; вони будують допоміжні структури, завдяки яким кожен наступний запит виконується швидко.">попередню обробку</Term> дерева, а потім $O(\log N)$ на кожен запит LCA.

:::tip[Коли підходить цей алгоритм?]
- Запити LCA онлайн, і вас влаштовує $O(\log N)$ на запит при простій реалізації? *(якщо всі запити відомі заздалегідь → офлайн [Тар'ян](lca_tarjan.md) за $O(1)$ у середньому)*
- Окрім LCA, потрібно підніматися на $k$ предків угору або шукати $k$-го предка / відповідати на запити на шляху (мінімум, сума)? *(саме для цього корисні таблиці двійкових підйомів)*
- Не критично вичавити теоретично оптимальні $O(1)$ на запит? *(якщо потрібні саме $O(1)$ при $O(N)$ передобчисленні → [Фарах-Колтон–Бендер](lca_farachcoltonbender.md))*
:::
...
Для кожної вершини ми також запам'ятаємо <Term tip="Лічильник кроків DFS у момент, коли обхід уперше заходить у вершину; разом із часом виходу він дозволяє швидко перевіряти відношення «предок-нащадок».">час першого відвідування</Term> цієї вершини (тобто момент, коли DFS відкриває вершину) і час, коли ми її залишили (тобто після того, як відвідали всіх нащадків і вийшли з функції DFS).
