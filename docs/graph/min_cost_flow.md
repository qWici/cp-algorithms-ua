...
Для кожного ребра (взагалі кажучи, орієнтованого, але див. нижче) задано <Term tip="максимальна кількість потоку, яку можна пропустити через це ребро.">пропускну здатність</Term> (невід'ємне ціле число) і вартість одиниці потоку вздовж цього ребра (деяке ціле число).
Також позначено <Term tip="витік — вершина, з якої потік виходить; стік — вершина, в яку потік приходить.">витік</Term> $s$ та стік $t$.
...
- Кожне ребро має не лише пропускну здатність, а й **вартість** одиниці потоку, і потрібно мінімізувати сумарну вартість? *(якщо вартостей немає, потрібен лише максимальний потік → [Дініц](dinic.md))*
...
для кожного ребра $(i, j)$ додаємо до мережі **<Term tip="допоміжне ребро у зворотному напрямку — воно дозволяє «скасувати» вже пущений потік, якщо пізніше виявиться вигіднішим інший шлях.">обернене ребро</Term>** $(j, i)$ з пропускною здатністю $U_{j i} = 0$ і вартістю $C_{j i} = -C_{i j}$.
...
Ми визначаємо **<Term tip="граф із ребер, через які ще можна пустити потік, із вказаним залишком пропускної здатності кожного ребра.">остаточну мережу</Term>** для деякого фіксованого потоку $F$ так (як і в алгоритмі Форда-Фалкерсона):
...
Оскільки описаний вище алгоритм потоку мінімальної вартості породжує обернене ребро для кожного орієнтованого ребра, він розщеплює неорієнтоване ребро на $4$ орієнтовані ребра, і ми фактично отримуємо **<Term tip="граф, у якому між однією й тією ж парою вершин може бути кілька паралельних ребер.">мультиграф</Term>**.
