...
**Центроїдна декомпозиція** — це техніка <Term tip="Загальний підхід: задачу розбивають на менші частини, розв'язують кожну окремо, а потім поєднують результати.">«розділяй і володарюй»</Term> на деревах. Її застосовують для розв'язання різноманітних задач, пов'язаних зі шляхами в дереві: підрахунку шляхів із певними властивостями, знаходження відстаней чи відповідей на запити щодо шляхів у дереві.
...
Для будь-якого заданого дерева з $N$ вершин існує один або два центроїди. Якщо центроїдів два, то вони обов'язково <Term tip="Дві вершини суміжні, якщо вони з'єднані ребром безпосередньо (є сусідами).">суміжні</Term>.
...
1. Обчислюємо <Term tip="Розмір піддерева вершини — це кількість вершин у частині дерева, що «звисає» з неї, разом із нею самою.">розміри піддерев</Term> для всіх вершин за допомогою пошуку в глибину (DFS)
