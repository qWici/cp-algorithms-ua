...
- Дерево **статичне** за структурою (змінюються лише кольори ребер), тож вистачає <Term tip="запис вершин дерева в тому порядку, у якому обхід у глибину їх проходить, із повторами при кожному поверненні назад.">ейлерового обходу</Term> + дерева відрізків?
...
Обидва ці запити мають особливу структуру: перша вершина є <Term tip="предок вершини — будь-яка вершина, що лежить вище за неї на шляху до кореня дерева.">предком</Term> другої.
До кінця статті ми говоритимемо лише про такий особливий вид запитів.

Почнімо з опису кроку **попередньої обробки**.
Запустимо <Term tip="обхід графа, який заглиблюється якомога далі вздовж кожної гілки, перш ніж повертатися назад.">пошук у глибину</Term> з кореня дерева й запишемо ейлерів обхід цього пошуку в глибину (кожну вершину додаємо до списку, коли обхід відвідує її вперше, а також щоразу, коли ми повертаємося з одного з її нащадків).
