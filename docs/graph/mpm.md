...
Подібно до алгоритму Дініца, MPM працює фазами, і протягом кожної фази ми знаходимо <Term tip="потік, який більше не можна збільшити вздовж жодного найкоротшого шляху від витоку до стоку — кожен такий шлях має хоча б одне повністю заповнене ребро.">блокуючий потік</Term> у <Term tip="граф, розбитий на шари за відстанню від витоку; у ньому ребра ведуть лише з одного шару в наступний.">шаруватій мережі</Term> <Term tip="граф із ребер, через які ще можна пустити потік, із залишком пропускної здатності кожного ребра.">залишкової мережі</Term> графа $G$.
...
Усі повністю <Term tip="дуга (орієнтоване ребро), заповнена потоком до межі пропускної здатності — крізь неї більше нічого не пропустиш.">насичені дуги</Term> можна видалити з $L$, оскільки вони все одно не використовуватимуться далі в цій фазі.
