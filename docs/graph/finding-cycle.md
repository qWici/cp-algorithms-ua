...
Розглянемо <Term tip="В орієнтованому графі кожне ребро має напрямок (вулиці з одностороннім рухом); у неорієнтованому ребром можна йти в обидва боки.">орієнтований або неорієнтований граф</Term> без <Term tip="Петля — це ребро, що з'єднує вершину саму із собою.">петель</Term> і <Term tip="Кратні (паралельні) ребра — це кілька ребер, що з'єднують одну й ту саму пару вершин.">кратних ребер</Term>. Нам потрібно перевірити, чи є він <Term tip="Ациклічний граф — це граф без жодного циклу: рухаючись ребрами, у ньому неможливо повернутися у вихідну вершину.">ациклічним</Term>, і якщо ні, то знайти будь-який цикл.
...
- Граф незважений, тобто цикл шукають за наявністю, а не за вагою? *(якщо шукають цикл **від'ємної ваги** у зваженому графі → [Пошук від'ємного циклу](finding-negative-cycle-in-graph.md))*
- Достатньо одного проходу [DFS](depth-first-search.md) за $O(M)$ без петель і кратних ребер?
:::
