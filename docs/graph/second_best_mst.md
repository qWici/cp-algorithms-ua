...
- Потрібне саме **друге за вагою** кістякове дерево (а не мінімальне)? *(якщо потрібне просто мінімальне → [Крускал](mst_kruskal.md))*
- Уже вмієш будувати MST і готовий зробити заміну одного ребра?
- Для швидкого варіанта через цикли доступний [LCA з двійковим підйомом](lca_binary_lifting.md) для пошуку найважчого ребра на шляху?
:::
...
1. Сортуємо ребра за $O(E \log E)$, потім знаходимо MST за допомогою алгоритму Краскала за $O(E)$.
2. Для кожного ребра $e$, якого ще немає в MST, тимчасово додаємо його до MST, <Term tip="у дереві будь-яке нове ребро замикає рівно один цикл — замкнений шлях, що повертається у вихідну вершину.">утворюючи цикл</Term>. Цикл проходитиме через LCA.
...
Один зі способів ефективно обчислити їх за $O(E \log V)$ — звести задачу до задачі про <Term tip="для двох вершин дерева — найглибша вершина, яка є предком обох одночасно.">найнижчого спільного предка (LCA)</Term>.

Ми виконаємо попередню обробку для LCA, <Term tip="оберемо одну вершину коренем і зорієнтуємо дерево від нього вниз, щоб у кожної вершини з'явилися чіткі предки.">підвісивши MST за корінь</Term>, а також обчислимо максимальні ваги ребер для кожної вершини на шляхах до її предків.
