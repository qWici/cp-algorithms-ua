...
Декартове дерево масиву `A` — це двійкове дерево з <Term tip="у дереві-купі кожна вершина не більша за своїх дітей, тож найменше значення завжди опиняється в корені.">властивістю min-купи</Term> (значення батьківської вершини має бути меншим або рівним значенню її дітей), таке, що <Term tip="обхід, у якому спершу відвідуємо ліве піддерево, потім саму вершину, а далі праве піддерево.">симетричний обхід (in-order)</Term> дерева відвідує вершини в тому самому порядку, у якому вони стоять у масиві `A`.
...
Запит мінімуму на відрізку `[l, r]` еквівалентний запиту <Term tip="для двох вершин дерева — найглибша вершина, яка є предком обох одночасно.">найнижчого спільного предка</Term> `[l', r']`, де `l'` — це вершина, що відповідає елементу `A[l]`, а `r'` — вершина, що відповідає елементу `A[r]`.
