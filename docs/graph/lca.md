...
- Запити LCA надходять онлайн (відповідати треба одразу), і ви хочете базовий підхід через зведення до RMQ на ейлеровому обході + [дерево відрізків](../data_structures/segment_tree.md) ($O(\log N)$) чи кореневе розбиття ($O(\sqrt{N})$)?
- Усі запити відомі заздалегідь (офлайн)? *(тоді простіший і швидший → [офлайн-алгоритм Тар'яна](lca_tarjan.md) за $O(1)$ у середньому на запит)*
- Потрібні теоретично оптимальні $O(1)$ на запит при $O(N)$ передобчисленні? *(тоді → [Фарах-Колтон–Бендер](lca_farachcoltonbender.md); якщо хочете простіший онлайн-метод з підйомом на $k$ предків → [двійкові підйоми](lca_binary_lifting.md))*
:::

### Ідея алгоритму \{#the-idea-of-the-algorithm}

Перш ніж відповідати на запити, нам потрібно виконати **<Term tip="Підготовчі обчислення, які роблять один раз до запитів; вони будують допоміжні структури, завдяки яким кожен наступний запит виконується швидко.">попередню обробку</Term>** дерева.
...
А в ейлеровому обході ми, по суті, використовуємо найкоротший шлях, окрім того, що додатково відвідуємо всі <Term tip="Піддерево вершини — це сама вершина разом з усіма її нащадками, тобто частина дерева, що «звисає» з неї.">піддерева</Term>, які трапляються нам на шляху.
