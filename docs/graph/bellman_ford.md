...
Припустимо, нам дано <Term tip="Граф, у якому кожне ребро має напрямок і число-вагу (наприклад, вартість переходу); рухатися ребром можна лише в заданому напрямку.">зважений орієнтований граф</Term> $G$ з $n$ вершинами та $m$ ребрами, а також деяку задану вершину $v$. Ми хочемо знайти довжини найкоротших шляхів від вершини $v$ до всіх інших вершин.

На відміну від алгоритму Дейкстри, цей алгоритм можна застосовувати й до графів, що містять ребра від'ємної ваги. Однак якщо граф містить <Term tip="Замкнений маршрут, сума ваг ребер якого від'ємна. Кружляючи ним, можна нескінченно зменшувати довжину шляху, тому найкоротший шлях перестає бути визначеним.">цикл від'ємної ваги</Term>, то, очевидно, найкоротшого шляху до деяких вершин може й не існувати (адже вага найкоротшого шляху має дорівнювати мінус нескінченності); проте цей алгоритм можна модифікувати так, щоб він сигналізував про наявність циклу від'ємної ваги або навіть відновлював цей цикл.
...
- У графі є ребра від'ємної ваги? *(якщо всі ваги невід'ємні → швидший [Дейкстра](dijkstra.md))*
- Потрібні найкоротші шляхи з однієї вершини або детекція від'ємного циклу? *(якщо потрібні всі пари при малому $V$ → [Флойд-Воршелл](all-pair-shortest-path-floyd-warshall.md); докладніше про пошук від'ємного циклу → [окрема стаття](finding-negative-cycle-in-graph.md))*
- Граф достатньо малий, щоб витримати $O(VE)$ (для прискорення на практиці є [варіант SPFA](#shortest-path-faster-algorithm-spfa) нижче)?
:::
