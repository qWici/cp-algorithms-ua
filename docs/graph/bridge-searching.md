...
Нам задано <Term tip="Граф, у якому ребра не мають напрямку: якщо вершини з'єднані, переходити між ними можна в обидва боки.">неорієнтований граф</Term>. Мостом називається таке ребро, видалення якого робить граф незв'язним (точніше кажучи, збільшує кількість <Term tip="Компонента зв'язності — максимальна група вершин, у якій від будь-якої можна дійти до будь-якої іншої; граф може складатися з кількох таких незалежних груп.">компонент зв'язності</Term> графа). Завдання полягає в тому, щоб знайти всі мости в заданому графі.
...
- Граф заданий повністю заздалегідь (статичний), а мости треба знайти один раз? *(якщо ребра додаються по одному й мости треба перераховувати після кожного додавання → [Пошук мостів онлайн](bridge-searching-online.md))*
- Потрібні саме **ребра**, видалення яких роз'єднує граф? *(якщо потрібні **вершини** → [Точки зчленування](cutpoints.md))*
- Достатньо одного проходу [DFS](depth-first-search.md) за $O(N+M)$?
:::
...
Для випадків <Term tip="Кратні (паралельні) ребра — це кілька ребер, що з'єднують одну й ту саму пару вершин.">кратних ребер</Term> треба бути обережними, ігноруючи ребро до батька. Щоб розв'язати цю проблему, ми можемо додати прапорець `parent_skipped`, який гарантує, що ми пропустимо батька лише один раз.
