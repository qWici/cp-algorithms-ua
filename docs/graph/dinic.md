...
Алгоритм Дініца розв'язує задачу про <Term tip="Найбільша кількість «потоку» (наприклад, води чи вантажу), яку можна провести від джерела до стоку мережею, не перевищуючи пропускну здатність жодного ребра.">максимальний потік</Term> за $O(V^2E)$. Задачу про максимальний потік описано в статті [Максимальний потік — Форда-Фалкерсона та Едмондса-Карпа](edmonds_karp.md). Цей алгоритм відкрив Юхим Дініц у 1970 році.
...
- $(v, u)$ з <Term tip="Максимальна кількість потоку, яку дозволено пропустити через це ребро — як максимальна «пропускна здатність» труби.">пропускною здатністю</Term> $c_{vu}^R = c_{vu} - f_{vu}$
