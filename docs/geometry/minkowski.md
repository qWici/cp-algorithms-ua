...
Тут ми розглядатимемо випадок, коли $A$ і $B$ складаються з <Term tip="многокутник без «вм’ятин»: відрізок між будь-якими двома його точками цілком лежить усередині; усі кути напрямлені назовні">опуклих многокутників</Term> $P$ і $Q$ разом з їхніми внутрішніми областями.
...
і $\{\overrightarrow{Q_jQ_{j+1}}\}$, упорядковані за <Term tip="кут, під яким напрямлений вектор відносно додатного напряму осі x; упорядкування за ним означає сортування ребер за напрямком, у який вони «дивляться»">полярним кутом</Term>. Ми стверджуємо, що послідовність ребер $P + Q$ можна отримати, злиявши
ці дві послідовності зі збереженням порядку за полярним кутом і замінивши послідовні <Term tip="вектори, що дивляться в один бік (мають однаковий напрямок), хоча можуть мати різну довжину">співнапрямлені вектори</Term> на їхню суму. Прямолінійне застосування цієї ідеї дає
...
замість обчислення полярних кутів і безпосереднього їх порівняння ми можемо дивитися на знак <Term tip="число, обчислене з координат двох векторів; його знак показує, чи повертає другий вектор відносно першого ліворуч, праворуч, чи вони на одній прямій">векторного добутку</Term> двох векторів.
