...
Маємо $n$ точок $P$ і хочемо знайти пару точок $p,q$, які найбільш віддалені одна від одної, тобто максимізувати $|x_p - x_q| + |y_p - y_q|$.
...
Тут ми показуємо спосіб знайти MST за $O(n \log{n})$, відшукуючи для кожної точки її найближчого сусіда в кожному октанті, як показано на зображенні нижче. Це дасть нам $O(n)$ кандидатних ребер, які, як ми покажемо нижче, гарантовано містять MST. Останній крок — застосувати якийсь стандартний алгоритм пошуку MST, наприклад, [алгоритм Краскала з використанням системи неперетинних множин](/graph/mst_kruskal_with_dsu).
...
Ми застосуємо підхід із замітальною прямою. Ми обробляємо точки з південного заходу на північний схід, тобто за неспадним $x + y$. Ми також тримаємо множину точок, які ще не мають свого найближчого сусіда, і називаємо її «активною множиною». Нижче ми додаємо зображення, які допоможуть уявити алгоритм.
