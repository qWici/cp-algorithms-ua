...
- Об'єкти — **нескінченні прямі**, а не відрізки скінченної довжини? *(якщо ні → [Перевірка перетину двох відрізків](check-segments-intersection.md))*
- Прямі задані (або легко зводяться) до загального вигляду $Ax + By + C = 0$?
...
Щоб знайти їхню точку перетину, нам треба розв'язати таку <Term tip="Кілька рівнянь, у яких невідомі входять лише в першому степені (без квадратів тощо); розв'язком є набір значень, що задовольняє всі рівняння одночасно.">систему лінійних рівнянь</Term>:
...
Скориставшись <Term tip="Правило Крамера — це готова формула для розв'язання системи лінійних рівнянь через визначники; вона одразу дає значення кожної невідомої.">правилом Крамера</Term>, ми можемо одразу виписати розв'язок системи, який дасть нам шукану точку перетину прямих:
...
Для цього достатньо обчислити такі <Term tip="Визначник 2 на 2 — це число, що обчислюється з чотирьох коефіцієнтів за формулою a·d мінус b·c; його рівність нулю означає пропорційність рядків.">визначники</Term>, і якщо вони обидва дорівнюють $0$, то прямі збігаються:
