...
- Тобі справді потрібна оболонка точок, а не оптимізація ДП виду $\min_j(k_j x + b_j)$? *(якщо ні → [Трюк опуклої оболонки](convex_hull_trick.md))*
- Усі точки відомі заздалегідь (<Term tip="Офлайн означає, що всі вхідні дані відомі від самого початку, тож їх можна обробити разом. Протилежне — онлайн, коли точки надходять по одній і відповідь треба підтримувати на льоту.">офлайн</Term>), тож достатньо одного проходу за $O(N \log N)$?
:::
...
Далі всі інші точки сортуються за <Term tip="Полярний кут точки — це кут, під яким її видно з опорної точки P0; сортування за ним розставляє точки в порядку обходу навколо P0.">полярним кутом</Term> за годинниковою стрілкою.
...
Якщо під час обходу Грема потрібно врахувати <Term tip="Колінеарні точки — це точки, що лежать на одній прямій. Тут ідеться про точки, які потрапляють рівно на сторону оболонки, а не лише у її вершини.">колінеарні точки</Term>, знадобиться
...
Однак це допускає <Term tip="Особлива крайня ситуація, що випадає із загальної логіки і потребує окремої обробки; тут — коли всі точки лежать на одній прямій і справжньої оболонки немає.">вироджений випадок</Term>, коли всі вхідні точки колінеарні на одній прямій, і алгоритм видав би повторювані точки.
