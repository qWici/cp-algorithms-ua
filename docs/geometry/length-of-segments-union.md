...
Нам потрібно знайти довжину їхнього <Term tip="Об'єднання відрізків — це множина всіх точок, покритих хоча б одним відрізком. Її довжину рахують один раз, навіть якщо відрізки накладаються.">об'єднання</Term>.

Наведений нижче алгоритм запропонував Klee у 1977 році.
Він працює за $O(n\log n)$ і доведено, що він <Term tip="Асимптотично оптимальний означає, що швидшого за порядком зростання алгоритму для цієї задачі не існує; покращити можна хіба що сталий множник.">асимптотично оптимальний</Term>.
