...
- Допустима <Term tip="Обчислення з дробовими числами (тип double), що дають наближений результат із крихітними неточностями. Тут вони неминучі через квадратні корені.">арифметика з рухомою комою</Term> (квадратні корені), а не лише цілочислові обчислення?
:::

## Розв'язок \{#solution}

Замість того щоб розв'язувати систему з двох рівнянь, ми підійдемо до задачі геометрично. У такий спосіб ми отримаємо точніший розв'язок з погляду <Term tip="Чисельна стійкість — це властивість методу не накопичувати великих похибок під час обчислень з дробовими числами; стійкіший метод дає точніший результат.">чисельної стійкості</Term>.

<Term tip="Зручне спрощення задачі: припускаємо найпростіше розташування (центр у початку координат), що не звужує розв'язок, бо решта випадків зводяться до нього зсувом.">Без втрати загальності</Term> вважаємо, що коло має центр у початку координат. Якщо це не так, ми переносимо його туди й коригуємо сталу $C$ у рівнянні прямої. Отже, маємо коло з центром у точці $(0,0)$ радіуса $r$ і пряму з рівнянням $Ax+By+C=0$.
...
Зауважимо, що вектор $(-B, A)$ <Term tip="Колінеарний означає «напрямлений уздовж того самого напрямку»: цей вектор паралельний прямій, тобто вказує її напрямок.">колінеарний</Term> прямій, а отже, ми можемо знайти шукані точки, додаючи й віднімаючи до точки $(x_0, y_0)$ вектор $(-B,A)$, відмасштабований до довжини $d$.
