...
Можна побачити, що ця множина точок — це саме множина точок, для яких проєкція на $\mathbf a$ є точкою $C \cdot \dfrac{\mathbf a}{|\mathbf a| ^ 2}$, і вони утворюють <Term tip="Гіперплощина — це узагальнення прямої та площини на будь-яку вимірність: у 2D це звичайна пряма, у 3D — площина.">гіперплощину</Term>, ортогональну до $\mathbf a$.
...
Це означує векторний добуток $\mathbf b\times \mathbf c$ векторів $\mathbf b$ та $\mathbf c$, а також <Term tip="Мішаний добуток трьох векторів — це число, що дорівнює об'єму паралелепіпеда, побудованого на цих векторах (зі знаком). Обчислюється як скалярний добуток першого вектора на векторний добуток двох інших.">мішаний добуток</Term> $\mathbf a\cdot(\mathbf b\times \mathbf c)$ векторів $\mathbf a$, $\mathbf b$ та $\mathbf c$.
...
Деякий аналог векторного добутку (а саме <Term tip="Псевдоскалярний добуток двох векторів на площині — це число (а не вектор), що дорівнює площі паралелограма на цих векторах зі знаком: плюс, якщо поворот від першого до другого вектора йде проти годинникової стрілки, мінус — якщо за нею.">псевдоскалярний добуток</Term>) можна реалізувати й у двовимірному випадку.
