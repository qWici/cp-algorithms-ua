...
тож якщо ми хочемо обчислити її за деяким <Term tip="Усі обчислення робимо, залишаючи лише остачу від ділення на m. Це тримає числа малими й не дає їм переповнитися.">простим модулем</Term> $m > n$, то отримуємо
...
Ми навіть можемо обчислити біноміальний коефіцієнт за час $O(1)$, якщо попередньо обчислимо <Term tip="Замість ділення за модулем множать на обернений елемент — таке число, що при множенні дає 1 за модулем. Це аналог ділення у світі остач.">обернені елементи</Term> всіх факторіалів за $O(\text{MAXN} \log m)$ звичайним методом обчислення оберненого елемента, або навіть за час $O(\text{MAXN})$, використовуючи конгруенцію $(x!)^{-1} \equiv ((x-1)!)^{-1} \cdot x^{-1}$ та метод [обчислення всіх обернених елементів](../algebra/module-inverse.md#mod-inv-all-num) за $O(n)$.
...
Але якщо $p \le \max(k, n-k)$, то принаймні один з факторіалів $k!$ і $(n-k)!$ не є <Term tip="Два числа взаємно прості, якщо в них немає спільних дільників, крім одиниці. Обернений елемент за модулем існує лише для взаємно простих чисел.">взаємно простим</Term> з $m$, а тому ми не можемо обчислити обернені елементи — вони не існують.
...
Це дає нам $h$ різних <Term tip="Конгруенція — це рівність остач: запис того, якій остачі дорівнює шукане число за кожним з модулів. Маючи кілька таких рівностей, можна відновити саме число.">конгруенцій</Term>.
