...
- Чи треба *перелічити* (вивести/перебрати) самі підмножини розміру $K$, а не лише дізнатися їхню кількість? *(якщо потрібна лише кількість → [Біноміальні коефіцієнти](binomial-coefficients.md))*
- Чи кількість комбінацій $\binom{N}{K}$ достатньо мала, щоб перебір був реальним за часом?
...
Спершу ми згенеруємо їх у <Term tip="Порядок «як у словнику»: набори порівнюються елемент за елементом зліва направо, і перша розбіжність визначає, котрий «менший».">лексикографічному порядку</Term>.
...
якщо ми зіставимо кожній підмножині <Term tip="Число, біти якого позначають присутність елементів: одиниця на позиції i означає, що i-й елемент входить у підмножину, нуль — що ні.">бітову маску</Term>, то, генеруючи й перебираючи ці бітові маски за допомогою кодів Грея, можемо отримати нашу відповідь.
...
Нижче наведено <Term tip="Найпростіший, прямолінійний спосіб без оптимізацій: тут просто перебираємо геть усі підмножини й відбираємо потрібні.">наївну реалізацію</Term>, що працює шляхом генерування всіх $2^{n}$ можливих підмножин і знаходження підмножин розміру
$K$.
