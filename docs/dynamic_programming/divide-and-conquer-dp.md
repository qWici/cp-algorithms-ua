...
- Рекурентне співвідношення має вигляд $dp(i, j) = \min_{k} \\{ dp(i - 1, k - 1) + C(k, j) \\}$, а прямолінійне ДП дає $O(m n^2)$, яке потрібно прискорити?
- Чи виконується умова монотонності $opt(i, j) \leq opt(i, j + 1)$ (зокрема, якщо функція вартості $C$ задовольняє нерівність чотирикутника $C(a, c) + C(b, d) \leq C(a, d) + C(b, c)$ для $a \leq b \leq c \leq d$)? *(якщо ні → монотонність не гарантована, оптимізація незастосовна)*
- Якщо вартість лінійна за параметром розбиття — чи не зручніше застосувати [трюк з опуклою оболонкою](../geometry/convex_hull_trick.md)?
:::

### Передумови \{#preconditions}
Деякі задачі динамічного програмування мають <Term tip="Формула, що задає відповідь для поточного стану через відповіді для менших станів. Саме за нею ДП поступово заповнює таблицю.">рекурентне співвідношення</Term> такого вигляду:

$$
dp(i, j) = \min_{0 \leq k \leq j} \\{ dp(i - 1, k - 1) + C(k, j) \\}
$$

де $C(k, j)$ — <Term tip="Функція, що задає «ціну» одного розбиття чи відрізка задачі — наприклад, вартість об'єднання елементів від k до j. Її конкретний вигляд залежить від задачі.">функція вартості</Term>, а $dp(i, j) = 0$ при $j \lt 0$.
...
функція вартості задовольняє <Term tip="Особлива властивість функції вартості: ціна двох «вкладених» відрізків не більша за ціну двох «перехресних». Саме вона гарантує, що оптимальна точка розбиття не «стрибає» назад.">нерівність чотирикутника</Term>, ми можемо показати, що
