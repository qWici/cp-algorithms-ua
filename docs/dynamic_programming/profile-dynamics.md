...
- Чи одна зі сторін ґратки $M$ мала (зазвичай $M \lesssim 15..20$), щоб маска рядка з $2^M$ станів вмістилася в пам'ять і час?
...
Ми будуватимемо стан ДП, перебираючи кожне $i = 1, \cdots N$ і кожну маску $mask = 0, \ldots 2^M - 1$, і для кожної маски $mask$ ми робитимемо переходи лише вперед, тобто ми _додаватимемо_ фігури до поточної ґратки.
