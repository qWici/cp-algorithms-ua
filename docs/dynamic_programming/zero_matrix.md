...
- Чи шукаєте ви найбільшу за площею прямокутну (<Term tip="Сторони прямокутника паралельні рядкам і стовпцям матриці, тобто він не повернутий під кутом.">вирівняну по осях</Term>) підматрицю з однакових елементів (нулів)?
- Чи можна звести задачу до серії <Term tip="Стовпчикова діаграма: для кожного стовпця рядка зберігаємо висоту суцільного «стовпчика» нулів над ним, а тоді шукаємо найбільший прямокутник під цими стовпчиками.">гістограм по рядках</Term>, де для кожного рядка треба знайти найбільший прямокутник? *(якщо ні → потрібен інший підхід до геометрії області)*
- Чи влаштовує складність $O(nm)$ зі застосуванням <Term tip="Структура даних, що працює за принципом «останнім прийшов — першим пішов»: елементи додаються й знімаються лише з одного кінця (вершини).">стека</Term> для найбільшого прямокутника в гістограмі?
:::
