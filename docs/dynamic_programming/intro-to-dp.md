...
Час роботи цієї рекурсивної функції <Term tip="Час росте надзвичайно швидко: з кожним додатковим елементом обсяг роботи приблизно подвоюється. Уже для кількох десятків елементів обчислення стає практично нездійсненним.">експоненційний</Term> — приблизно $O(2^n)$, оскільки один виклик функції ( $f(n)$ ) породжує 2 виклики приблизно такого ж розміру ($f(n-1)$ та $f(n-2)$ ).
...
Ці альтернативні способи збереження стану корисні насамперед тоді, коли в <Term tip="Множина всіх можливих станів задачі — тобто всіх різних наборів значень, для яких ми обчислюємо й зберігаємо відповідь.">простір станів</Term> входять вектори або рядки.
...
Цей підхід називається згори вниз, оскільки ми можемо викликати функцію зі значенням-запитом, і обчислення починає рухатися згори (запитаного значення) вниз (до <Term tip="Найпростіші випадки, для яких відповідь відома одразу, без подальших викликів — тут це f(0)=0 і f(1)=1. На них рекурсія зупиняється.">базових випадків рекурсії</Term>), роблячи по дорозі скорочення через мемоїзацію.
