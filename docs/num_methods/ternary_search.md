...
Нам задано функцію $f(x)$, яка є <Term tip="Функція з одним «горбом»: спочатку лише зростає, а після точки максимуму лише спадає (або навпаки — для мінімуму). Без плато і другого піка.">унімодальною</Term> на відрізку $[l, r]$. Під унімодальною функцією ми розуміємо одну з двох можливих поведінок функції:
...
- Функція лише монотонна, а ви шукаєте точку переходу чи конкретне значення, а не екстремум? *(якщо так → [Бінарний пошук](binary_search.md))*
- Точку екстремуму можна знайти лише через порівняння значень $f$, без аналітичної похідної? *(якщо похідна відома й потрібна швидка збіжність → [Метод Ньютона](roots_newton.md))*
:::
...
Ми не накладали жодних обмежень на вибір точок $m_1$ та $m_2$. Цей вибір визначатиме <Term tip="Наскільки швидко відрізок пошуку стискається до відповіді — тобто скільки кроків потрібно для потрібної точності.">швидкість збіжності</Term> й точність реалізації. Найпоширеніший спосіб — вибрати точки так, щоб вони ділили відрізок $[l, r]$ на три рівні частини. Отже, маємо
