...
Нехай у нас є набір робіт, і для кожної роботи ми знаємо її <Term tip="Крайній момент часу, до якого роботу має бути завершено.">дедлайн</Term> та тривалість. Виконання роботи не можна перервати до її завершення. Потрібно скласти такий розклад, щоб виконати якнайбільше робіт.
...
- Чи штрафу за час очікування немає — інакше це задача про мінімізацію штрафу → [Планування робіт на одній машині](schedule_one_machine.md)?
:::

## Розв'язок \{#solving}

Алгоритм розв'язання — **<Term tip="Підхід, що на кожному кроці робить локально найкращий вибір, сподіваючись отримати найкращий результат у цілому.">жадібний</Term>**. Відсортуймо всі роботи за їхніми дедлайнами та переглядаймо їх у порядку спадання. Також створімо чергу $q$, у яку поступово додаватимемо роботи й діставатимемо ту, що має найменший час виконання (наприклад, можна використати set або priority_queue). Спочатку $q$ порожня.
