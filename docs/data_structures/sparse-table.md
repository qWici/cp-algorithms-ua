...
- Чи це ідемпотентна операція (на кшталт $\min$/$\max$/$\gcd$), для якої потрібна відповідь за $O(1)$ при перекритті відрізків?
- Якщо операція не ідемпотентна (наприклад, сума), чи влаштовує відповідь за $O(\log n)$, або ж потрібна $O(1)$ для асоціативної операції? *(для $O(1)$ → [Sqrt-дерево](sqrt-tree.md))*
:::
...
Вимір $\text{MAXN}$ стоїть другим, щоб дозволити (дружні до кешу) послідовні звернення до пам'яті.
...
Оскільки відрізок $[j, j + 2^i - 1]$ довжини $2^i$ гарно розбивається на відрізки $[j, j + 2^{i - 1} - 1]$ та $[j + 2^{i - 1}, j + 2^i - 1]$, обидва довжини $2^{i - 1}$, ми можемо ефективно згенерувати таблицю за допомогою динамічного програмування:
