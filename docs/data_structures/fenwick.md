...
Нехай $f$ — деяка групова операція (бінарна <Term tip="Властивість, коли результат не залежить від розставлення дужок: (a*b)*c дає те саме, що a*(b*c). Завдяки цьому елементи можна об'єднувати в будь-якому порядку зліва направо.">асоціативна</Term> функція над множиною з <Term tip="Особливий елемент, який нічого не змінює: для додавання це 0 (x + 0 = x), для множення — 1.">нейтральним елементом</Term> та <Term tip="Для кожного елемента існує «протилежний», який повертає до нейтрального: для додавання обернений до x — це -x. Саме це дозволяє «віднімати» й рахувати суму на відрізку.">оберненими елементами</Term>), а $A$ — масив цілих чисел довжини $N$.
...
- Чи є операція на відрізку оборотною (наприклад, сума за наявності віднімання), тобто утворює групу? *(якщо операція лише асоціативна, як $\min$/$\max$ без обернення → [дерево відрізків](segment_tree.md))*
- Чи потрібні оновлення елементів між запитами? *(якщо масив незмінний, а потрібен лише $\min$/$\max$ за $O(1)$ → [розріджена таблиця](sparse-table.md))*
...
Це відповідало б масивам <Term tip="Заздалегідь обчислений масив, де в комірці i зберігається сума всіх елементів від початку до i. Тоді суму будь-якого відрізка можна дістати одним відніманням.">префіксних сум</Term> (у такому разі знаходження суми на відрізку $[0, i]$ потребуватиме лише сталого часу; проте оновлення будуть повільними).
...
Іншими словами, якщо <Term tip="Останній (найправіший) біт у двійковому записі числа — той, що відповідає за парність: він дорівнює 1 у непарних чисел і 0 у парних.">наймолодший двійковий розряд</Term> числа $i$ дорівнює $0$, то $g(i) = i$.
