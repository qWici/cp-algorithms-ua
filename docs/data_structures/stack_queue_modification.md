...
- Чи потрібен мінімум (або максимум) на ковзному вікні фіксованої довжини за сумарний час $O(n)$? *(якщо вікно довільне й змінюється → [дерево відрізків](segment_tree.md) або [розріджена таблиця](sparse-table.md))*
...
Ми хочемо модифікувати структуру даних «стек» так, щоб можна було знаходити найменший елемент у стеку за час $O(1)$, зберігаючи при цьому ту саму асимптотику для додавання та видалення елементів зі стеку.
