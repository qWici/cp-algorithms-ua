...
Точніше, декартове дерево — це структура даних, яка зберігає пари $(X, Y)$ у бінарному дереві так, що воно є <Term tip="Дерево, де для кожного вузла всі менші ключі лежать у лівому піддереві, а всі більші — у правому. Завдяки цьому потрібний елемент шукається спуском від кореня.">бінарним деревом пошуку</Term> за $X$ і <Term tip="Дерево, де значення в кожному вузлі не менше (або не більше) за значення його нащадків, тож на вершині завжди лежить максимум (або мінімум).">бінарною купою</Term> за $Y$.
...
- Чи потрібні операції розрізання/злиття (split/merge) послідовності або вставка/видалення за довільною позицією, чого не вміє дерево відрізків? *(якщо потрібні лише агрегати на відрізку без зміни структури → [дерево відрізків](segment_tree.md))*
- Чи потрібне <Term tip="Дерево, у якому гілки приблизно однакової глибини, тож воно не вироджується в довгий ланцюг і операції залишаються швидкими — порядку логарифма.">збалансоване</Term> бінарне дерево пошуку з <Term tip="Можливість швидко питати «який елемент стоїть на k-й позиції за порядком» та обернено «який порядковий номер у заданого елемента».">порядковою статистикою</Term> ($k$-й елемент, індекс елемента), а не лише запити на масиві фіксованого розміру?
- Чи влаштовує очікувана збалансованість $O(\log n)$ за рахунок випадкових пріоритетів заради простоти реалізації?
:::
...
Декартове дерево за <Term tip="Замість зберігати ключ явно, за ключ беруть позицію елемента в масиві, яка обчислюється на льоту з розмірів піддерев. Так дерево поводиться як масив із швидкими вставками й видаленнями посередині.">неявним ключем</Term> — це проста модифікація звичайного декартового дерева, яка є дуже потужною структурою даних. Фактично декартове дерево за неявним ключем можна розглядати як масив з такими реалізованими процедурами (усі за $O (\log N)$ в <Term tip="Запити надходять і обробляються по одному в реальному часі: відповідь на кожен треба дати одразу, не знаючи майбутніх запитів.">онлайн-режимі</Term>):
