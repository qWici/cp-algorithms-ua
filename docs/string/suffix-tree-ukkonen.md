...
Цей алгоритм будує <Term tip="Суфіксне дерево — це дерево, у якому кожен шлях від кореня донизу читає якийсь суфікс рядка. Воно компактно зберігає всі суфікси й дозволяє швидко відповідати на запити про підрядки.">суфіксне дерево</Term> для заданого рядка $s$ довжини $n$ за час $O(n\log(k))$), де $k$ — <Term tip="Розмір алфавіту — це кількість різних символів, які можуть траплятися в рядку (наприклад, 26 для малих англійських літер).">розмір алфавіту</Term> (якщо вважати $k$ константою, асимптотика стає лінійною).
...
* `link` — <Term tip="Суфіксне посилання веде з вершини у вершину, що відповідає рядку, коротшому на один перший символ. Воно дозволяє алгоритму Укконена будувати дерево за лінійний час.">суфіксне посилання</Term>,
