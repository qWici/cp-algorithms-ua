...
- Задача **<Term tip="Лексикографічна означає, що рядки впорядковуються «як у словнику»: символ за символом зліва направо, і перша розбіжність вирішує, котрий менший.">лексикографічна</Term>** на суфіксах ($k$-тий підрядок, найдовший спільний підрядок, кількість різних підрядків через LCP)? *(якщо потрібні переважно підрядкові запити, а не лексикографічний порядок → [Суфіксний автомат](suffix-automaton.md))*
- Багато запитів до **одного** тексту, через що варто передобчислити структуру по його суфіксах? *(якщо взірців багато, а текст один прохід → [Ахо-Корасік](aho_corasick.md))*
- Потрібен саме впорядкований масив суфіксів (а не лінійні $O(n)$ методи на кшталт КМП/хешів для простих задач пошуку)? *(для простого пошуку одного взірця → [КМП](prefix-function.md))*
:::
...
Строго кажучи, наведений далі алгоритм сортуватиме не самі суфікси, а <Term tip="Циклічний зсув — це рядок, отриманий перенесенням кількох перших символів у кінець, ніби рядок записано по колу. Наприклад, зсуви слова abc — це bca і cab.">циклічні зсуви</Term> рядка.
...
Це можна зробити тривіально, наприклад, за допомогою **<Term tip="Сортування підрахунком розкладає елементи за значеннями, підраховуючи, скільки разів трапляється кожне. Працює за лінійний час, коли значень небагато.">сортування підрахунком</Term>**.
...
Як зазначено на початку цього розділу, ми можемо отримати відсортований порядок суфіксів, дописавши <Term tip="Спеціальний символ-вартовий (сентинель), менший за будь-який звичайний символ. Він гарантує, що порядок циклічних зсувів збігається з порядком суфіксів.">символ, менший за всі інші символи рядка</Term>, і відсортувавши отриманий рядок за циклічними зсувами, наприклад, відсортувавши циклічні зсуви $s + \textdollar$.
