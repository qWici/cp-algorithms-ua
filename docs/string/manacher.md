...
Загалом ця задача має багато розв'язків: за допомогою [хешування рядків](string-hashing.md) її можна розв'язати за $O(n\cdot \log n)$, а за допомогою [суфіксних дерев](suffix-tree-ukkonen.md) і швидкого <Term tip="LCA (найнижчий спільний предок) — це найглибша вершина дерева, що є предком одночасно двох заданих вершин. Швидкі запити LCA дозволяють миттєво порівнювати спільні частини суфіксів.">LCA</Term> цю задачу можна розв'язати за $O(n)$.

Але описаний тут метод **значно** простіший і має меншу <Term tip="Прихована стала — це множник, який ховається за позначенням O(...). Два алгоритми з однаковою складністю можуть на практиці працювати з різною швидкістю саме через цю сталу.">приховану сталу</Term> в складності за часом і пам'яттю. Цей алгоритм відкрив **Glenn K. Manacher** у 1975 році.

Ще один сучасний спосіб розв'язати цю задачу і працювати з паліндромами загалом — це так зване <Term tip="Паліндромне дерево (eertree) — це спеціальна структура даних, кожна вершина якої відповідає одному унікальному паліндрому-підрядку. Зручна для задач, де паліндромів багато.">паліндромне дерево, або eertree</Term>.
...
Для швидкого обчислення ми підтримуватимемо **<Term tip="Виключні межі означають, що самі позиції l і r не входять у паліндром — це його межі ззовні. Паліндром займає лише символи строго між ними.">виключні межі</Term> $(l, r)$** найправішого знайденого (під)паліндрома (тобто поточний найправіший (під)паліндром — це $s[l+1] s[l+2] \dots s[r-1]$). Спочатку ми встановлюємо $l = 0, r = 1$, що відповідає порожньому рядку.
