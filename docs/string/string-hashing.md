...
- Прийнятна **ймовірнісна** відповідь (рідкісні <Term tip="Колізія — це коли два різні рядки випадково дістають однаковий хеш, через що алгоритм може помилково вважати їх рівними.">колізії хешів</Term> допустимі)? *(якщо потрібна детермінована відповідь для одного взірця → [КМП](prefix-function.md))*
- Шукаєте взірець у тексті саме через хеші <Term tip="Ковзне вікно — це відрізок фіксованої довжини, який ми посуваємо по тексту на один символ за раз, щоразу швидко перераховуючи хеш замість обчислення з нуля.">ковзним вікном</Term>? *(тоді див. конкретну реалізацію → [Рабін–Карп](rabin-karp.md))*
:::
...
Використання хешування не буде на 100% <Term tip="Детерміновано правильний означає, що відповідь завжди точна, без жодної ймовірності помилки. З хешами така гарантія втрачається через можливі колізії.">детерміновано правильним</Term>, бо два цілком різні рядки можуть мати однаковий хеш (хеші колізують).
...
Тому нам потрібно знайти [обернений елемент за модулем](../algebra/module-inverse.md) до $p^i$, а потім виконати множення на цей <Term tip="Обернений елемент за модулем — це число, множення на яке грає роль ділення в арифметиці за модулем, бо просто ділити там не можна.">обернений елемент</Term>.
