...
- Потрібно **обчислити** математичний вираз із рядка з урахуванням <Term tip="Пріоритет визначає, які операції виконуються першими: наприклад, множення має вищий пріоритет за додавання, тому в 2 + 3 * 4 спершу множать.">пріоритетів операторів</Term> і дужок за $O(n)$?
- Граматика проста (бінарні/унарні оператори, дужки), тож достатньо алгоритму на <Term tip="Стек — це структура даних за принципом «останнім прийшов — першим пішов»: елементи кладуть і знімають лише з вершини, як стопку тарілок.">стеку</Term>, а не повноцінного <Term tip="Парсер — це програма, яка розбирає текст за правилами мови (граматикою) і будує його структуру. Тут вистачає простішого підходу без повного парсера.">парсера</Term>?
- Достатньо одного проходу зі стеками <Term tip="Операнд — це значення, над яким діє оператор. У виразі 2 + 3 числа 2 і 3 є операндами, а плюс — оператором.">операндів</Term> і операторів, без побудови <Term tip="Дерево розбору — це деревоподібне представлення виразу, де гілки показують, які операції над якими частинами виконуються. Тут ми обходимося без його явної побудови.">дерева розбору</Term>?
:::
...
<YouTubeEmbed id="Wz85Hiwi5MY" title="Comp Sci in 5: Shunting Yard Algorithm — Comp Sci in 5" />
