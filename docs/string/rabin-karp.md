...
Задача: дано два рядки — <Term tip="Це короткий рядок, який ми шукаємо всередині великого тексту. Інші назви — шаблон або зразок.">взірець</Term> $s$ і текст $t$. Потрібно визначити, чи трапляється взірець у тексті, і якщо так, то перелічити всі його входження за час $O(|s| + |t|)$.

Алгоритм: обчислюємо хеш для взірця $s$.
Обчислюємо значення хешів для всіх <Term tip="Префікс — це початковий шматок рядка: усі символи від першого до якоїсь позиції. Тут ми наперед рахуємо хеш кожного такого початкового шматка, щоб потім швидко діставати хеш будь-якого підрядка.">префіксів</Term> тексту $t$.
...
- Шукаєте **один** взірець у тексті й прийнятна ймовірнісна відповідь (рідкісні <Term tip="Колізія — це коли два різні рядки випадково мають однаковий хеш. Тоді алгоритм може помилково вирішити, що рядки збігаються.">колізії хешів</Term> допустимі)? *(якщо потрібен <Term tip="Детермінований означає, що алгоритм завжди дає точну правильну відповідь без жодної ймовірності помилки, на відміну від хешування, де є малий шанс колізії.">детермінований результат</Term> → [КМП](prefix-function.md))*
- Взірець лише один, а не цілий словник? *(якщо взірців багато одночасно → [Ахо-Корасік](aho_corasick.md))*
- Потрібні також інші порівняння довільних підрядків за $O(1)$? *(тоді корисна загальна техніка → [Хешування рядків](string-hashing.md))*
:::
