...
- Потрібен загальний розв'язок «розділяй і володарюй» за $O(n \log n)$ для всіх повторів, а не лише пошук конкретного взірця? *(якщо шукаєте задане слово → [КМП](prefix-function.md))*
- Не вистачає простіших засобів і потрібна саме структурна інформація про повтори у тексті? *(для загальних підрядкових запитів → [Суфіксний автомат](suffix-automaton.md))*
:::
