...
- Потрібен загальний розв'язок <Term tip="Підхід «розділяй і володарюй» розбиває задачу на менші частини, розв'язує кожну окремо (зазвичай рекурсивно) і потім поєднує результати.">«розділяй і володарюй»</Term> за $O(n \log n)$ для всіх повторів, а не лише пошук конкретного взірця? *(якщо шукаєте задане слово → [КМП](prefix-function.md))*
...
Загалом будь-який <Term tip="Періодичний рядок — це рядок, що утворюється багаторазовим повторенням якогось короткого шматка (періоду), наприклад abcabcabc із періодом abc.">періодичний рядок</Term> із коротким періодом міститиме багато повторів.
...
- Щоб попередньо обчислити всі значення $k_2$, обчислюємо Z-функцію для рядка $v + \# + u$ (тобто рядка $u$, з'єднаного з <Term tip="Спеціальний символ (сентинель), якого немає в самих рядках. Він ставиться між склеєними рядками, щоб збіг не міг випадково перетнути межу між ними.">символом-роздільником</Term> $\#$ і рядком $v$).
