...
Маємо масив $A$ розміру $N$ і число $K$. Задача полягає в тому, щоб знайти $K$-те за величиною число в масиві, тобто $K$-ту <Term tip="Елемент, який опинився б на K-му місці, якби масив відсортувати за зростанням.">порядкову статистику</Term>.
...
- Чи потрібен лише **один** ($K$-й) елемент за порядком, а не повне сортування масиву?
- Чи допустимо **переставляти** елементи масиву на місці (часткове впорядкування навколо <Term tip="Опорного елемента, відносно якого масив ділять на дві частини: менші за нього й більші.">бар'єра</Term>)?
- Чи влаштовує **середній** час $O(N)$ — або потрібна гарантія в найгіршому випадку (тоді бери <Term tip="Спеціальний спосіб вибору опорного елемента, що гарантує лінійний час навіть у найгіршому випадку: масив ділять на групи, беруть медіану кожної, а потім медіану цих медіан.">«медіану медіан»</Term>)?
:::
...
## Зауваження \{#notes}
* Наведений вище <Term tip="Алгоритм, що використовує випадковість (тут — випадковий вибір опорного елемента), завдяки чому добре працює в середньому.">рандомізований</Term> алгоритм називається [швидкий вибір (quickselect)](https://en.wikipedia.org/wiki/Quickselect). Перед його викликом слід випадково перемішати $A$ або використовувати випадковий елемент як бар'єр, щоб він працював коректно. Існують також детерміновані алгоритми, які розв'язують зазначену задачу за лінійний час, наприклад [медіана медіан](https://en.wikipedia.org/wiki/Median_of_medians).
