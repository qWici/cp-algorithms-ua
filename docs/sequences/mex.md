...
- Чи шукаєш саме **найменше невід'ємне відсутнє** число (а не, скажімо, $K$-й елемент за порядком — тоді → [$K$-та порядкова статистика](k-th.md))?
- Чи достатньо обчислити MEX **один раз** — тоді вистачить булевого масиву за $O(N)$?
- Чи масив **змінюється** між запитами — тоді потрібна структура (`set` / [дерево відрізків](../data_structures/segment_tree.md)) для $O(\log N)$ на оновлення?
:::
...
Якщо алгоритму потрібне обчислення MEX за $O(N)$, цього можна досягти, використавши <Term tip="Масив, у якому кожна комірка зберігає лише так чи ні (true або false); тут позначає, чи трапилося число.">булевий вектор</Term> замість множини.
...
Наприклад, дерево відрізків або <Term tip="Збалансоване двійкове дерево пошуку, яке підтримує швидкі запити та оновлення (його ще називають treap).">декартове дерево</Term>.
...
Таку структуру даних можна оновлювати за час $O(\log N)$, а також знаходити MEX за час $O(\log N)$, виконуючи <Term tip="Пошук, що на кожному кроці відкидає половину варіантів, спускаючись по дереву до потрібної відповіді.">бінарний пошук MEX</Term>.
