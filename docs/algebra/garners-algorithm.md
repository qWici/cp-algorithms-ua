...
- Чи задача зводиться до відновлення великого числа з його остач за набором попарно взаємно простих модулів? *(якщо ні → [Китайська теорема про остачі](chinese-remainder-theorem.md))*
- Чи модулі $p_i$ фіксовані наперед, тож можна один раз передобчислити обернені елементи $r_{ij}$ за час $O(k^2)$?
