...
Завдяки структурі <Term tip="Множини всіх остач за модулем, взаємно простих з модулем, де операцією є множення за цим модулем.">мультиплікативної групи за модулем</Term> $2^d$ будь-яке число $x$ таке, що $x \equiv 1 \pmod 4$, можна подати у вигляді
...
На практиці використовується дещо інший підхід, ніж описаний вище. Замість того щоб шукати <Term tip="Розклад числа на множники потрібного вигляду, тут — на добуток множників виду «два в степені плюс один».">факторизацію</Term> для $x$, ми послідовно множитимемо $x$ на $2^n+1$, доки не перетворимо його на $1$ за модулем $2^d$. Таким чином ми знайдемо подання $x^{-1}$, тобто
...
звідки (<Term tip="Прийом, коли потрібний степінь отримують, багаторазово підносячи проміжний результат до квадрата, що значно швидше за множення поспіль.">послідовним піднесенням до квадрата</Term>) можна вивести, що

$$
(2^a+1)^{2^b} \equiv 1 \pmod{2^{a+b}}.
$$

Застосувавши цей результат до $a=2^n+1$ та $b=d-k$, дістаємо, що <Term tip="Найменший додатний степінь, у якому число дає остачу 1 при множенні за заданим модулем.">мультиплікативний порядок</Term> числа $2^n+1$ є дільником $2^{d-n}$.
