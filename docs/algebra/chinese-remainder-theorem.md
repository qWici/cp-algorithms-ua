...
- Чи модулі $m_i$ попарно взаємно прості (інакше потрібен окремий розбір степенів простих, описаний у статті)? *(якщо модуль один і простий → [Лінійне конгруентне рівняння](linear_congruence_equation.md))*
...
Ми хочемо знайти розв'язок для $a \pmod{m_1 m_2}$. За допомогою [розширеного алгоритму Евкліда](extended-euclid-algorithm.md) ми можемо знайти коефіцієнти Безу $n_1, n_2$ такі, що
