...
У цій статті ми опишемо алгоритм «крок немовляти — крок велетня» (baby-step giant-step), запропонований Шенксом у 1971 році для обчислення дискретного логарифма, який має часову складність $O(\sqrt{m})$. Це алгоритм типу **<Term tip="Прийом, коли задачу ділять на дві половини, окремо перебирають варіанти кожної, а тоді шукають збіг між ними посередині — це різко зменшує загальний перебір.">«зустріч посередині»</Term> (meet-in-the-middle)**, оскільки він використовує прийом розбиття задачі навпіл.

:::tip[Коли підходить цей алгоритм?]
- Чи потрібно знайти показник $x$ у рівнянні $a^x \equiv b \pmod m$ (а не сам степінь)? *(якщо навпаки треба обчислити $a^x \bmod m$ → [Бінарне піднесення до степеня](binary-exp.md))*
- Чи модуль $m$ достатньо малий, щоб $O(\sqrt{m})$ кроків і пам'ять під хеш-таблицю вкладалися в ліміти (зазвичай $m \lesssim 10^{12}$…$10^{14}$)?
- Якщо це рівняння $x^k \equiv a \pmod n$ відносно основи, а не показника — *(→ [Дискретний корінь](discrete-root.md))*?
:::
...
де $a$ і $m$ <Term tip="Не мають спільних дільників, окрім одиниці: їхній найбільший спільний дільник дорівнює 1.">взаємно прості</Term>.
...
Усередині `map` використовує <Term tip="Самобалансоване двійкове дерево пошуку, яке тримає висоту малою, тож вставка, пошук і видалення працюють за логарифмічний час.">червоно-чорне дерево</Term> для зберігання значень.
...
Замість `map` ми можемо також використати <Term tip="Структуру, що зберігає пари «ключ — значення» і знаходить їх майже миттєво, обчислюючи за ключем позицію в масиві за допомогою хеш-функції.">хеш-таблицю</Term> (`unordered_map` у C++), яка має середню часову складність $O(1)$ для вставки та пошуку.
