...
можна використовувати з будь-якими операціями, що мають властивість **<Term tip="Властивість операції, за якої порядок групування доданків чи множників не впливає на результат: можна спершу обчислити будь-яку пару сусідніх елементів.">асоціативності</Term>**:
...
Ідея бінарного піднесення до степеня в тому, що ми розбиваємо роботу, використовуючи <Term tip="Запис показника степеня в двійковій системі: число подають як суму степенів двійки, що відповідають його одиничним бітам.">двійкове представлення показника</Term>.
...
Це безпосередньо випливає з <Term tip="Якщо m просте і a не ділиться на m, то a у степені m мінус один дає остачу 1 за модулем m. Це дозволяє зменшувати показник степеня за модулем.">малої теореми Ферма</Term> та <Term tip="Узагальнення теореми Ферма на будь-який модуль: a у степені функції Ейлера від m дає остачу 1 за модулем m, якщо a і m взаємно прості.">теореми Ейлера</Term>, докладніше див. статтю про [обернені елементи за модулем](module-inverse.md#fermat-euler).
...
**Розв'язок:** Ця задача розглядається докладніше в [окремій статті](../graph/fixed_length_paths.md). Алгоритм полягає в піднесенні <Term tip="Квадратна таблиця, де клітинка в рядку i та стовпці j показує, чи є ребро з вершини i до вершини j графа.">матриці суміжності</Term> $M$ графа (матриці, де $m_{ij} = 1$, якщо є ребро з $i$ до $j$, або $0$ в іншому випадку) до $k$-го степеня. Тоді $m_{ij}$ буде кількістю шляхів довжини $k$ з $i$ до $j$. Часова складність цього розв'язку — $O(n^3 \log k)$.
