...
- Чи потрібно ділити за модулем у задачі, де модуль $m$ не обов'язково простий? Тоді підходить [розширений алгоритм Евкліда](extended-euclid-algorithm.md) (формула $a^{m-2}$ через [бінарне піднесення](binary-exp.md) працює лише для простого $m$).
