...
- Чи потрібно обчислити кількість $d(n)$ або суму $\sigma(n)$ дільників числа через його розклад на прості множники?
- Чи запитів небагато, щоб факторизація кожного $n$ за $O(\sqrt{n})$ була прийнятною? *(якщо потрібно $d(n)$ чи $\sigma(n)$ для всіх $n$ до $N$ одразу → [Решето Ератосфена](sieve-of-eratosthenes.md))*
- Якщо саме отримання розкладу на множники складне (велике $n$) — *(→ [Факторизація цілих чисел](factorization.md))*?
:::
